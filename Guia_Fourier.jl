...
# ╔═╡ a463d4c1-76cb-49ce-8127-07093735104c
@bind amp Slider(5:15)

# ╔═╡ 2410c1cf-0c60-44d2-835b-e4af6a695a25
@bind w Slider(6:5000)

# ╔═╡ 6e6ae19a-99bb-49fc-b961-5d64e589bce7
wave, t₀, y₀ = ej1(amp, w)

# ╔═╡ 50530c67-a95d-4eb3-95fa-d3376ee204e3
plot(t₀[1:w], y₀[1:w])

# ╔═╡ f876cc56-8359-4c5e-8595-965a7849bae9
function ej2()
	freqs = [1.55*2^(n) for n in 4:11]
...
# ╔═╡ c6c625f2-156a-46f9-83f3-125f0f37f95d
ej2()

# ╔═╡ 2c0c1554-113e-4833-abda-2351c81bd339
function int_trapecio(f, interval, h)
	points = interval[1]:h:interval[2]
	vals = [
		(h/2)*f(interval[1]),
		[h*f(x) for x in points[2:length(points)-1]]...,
		(h/2)*f(interval[2])
	]
...
# ╔═╡ d1ccd82c-105d-43e7-ae4b-baab0a82c8b8
int_trapecio(x -> x^2, (-1, 1), 1e-6)

# ╔═╡ 7b78b4da-67c7-452e-8395-516353f2a13a
int_trapecio(x -> exp(-x^2), (-10, 10), 1e-3)^2

# ╔═╡ bdbe3285-6f7c-4825-99b3-265859194273
function dirichlet(N, t)
	return sin((N+(1/2))*t)/(2*sin(t/2))
end

# ╔═╡ 988e8fd6-e459-4f3f-9604-dc286f456b51
@bind N₁ Slider(1:15)

# ╔═╡ 66941d3b-7b85-40b5-94ad-b9f1e4ae00f1
plot(dirichlet.(N₁, -π:1e-5:π))

# ╔═╡ 115ac37d-a03d-4d8e-9920-17b4229482eb
(1/π)*int_trapecio(t->dirichlet(N₁, t), (-π, π), 1e-4)

# ╔═╡ e1e6051f-6520-44b3-8023-fcd6cd869443
function coeffs(N, f)
	a = (1/π)*int_trapecio(t->f(t)*cos(N*t), (-π, π), 1e-4)
	b = (1/π)*int_trapecio(t->f(t)*sin(N*t), (-π, π), 1e-4)
