...
# ╔═╡ 2f5d2218-4103-11ee-3ced-ed3a41767956
using Plots, DifferentialEquations

# ╔═╡ 5f816fce-08c7-4308-b0e3-d5c67f35f654
include("clase2.jl")

# ╔═╡ 803b3ac5-545d-43e4-9269-a29099f454ee
md"""## `DifferentialEquations`"""

# ╔═╡ 2ba032f2-14fe-4425-9be9-69ff14a9b033
md"""La librería `DifferentialEquations` funcion a partir de la definición de un problema. En nuestro caso, un `ODEProblem`. La sintaxis es:

	prob = ODEProblem(f,u₀,tspan[,p])

donde `f=f(u,p,t)` es la función que define el sistema y devuelve la derivada de `u` evaluada a tiempo `t` y de acuerdo a los parámetros `p`. 

Una vez definido el problema podemos resolverlo haciendo simplemente: 

	sol = solve(prob)


Consideremos, por ejemplo, la ecuación logística con la que trabajamos antes. Podemos tomar el archivo en el que la definimos, para no tener que reescribirla. """

# ╔═╡ c013a64b-ae72-4d01-b87e-2cc7b0387b96
prob_logistica = ODEProblem(logistica,20_000_000,(0,10),[0.5,50_000_000])

# ╔═╡ 6e28263f-505d-4abd-a432-45a4b580d4ab
sol_logistica  = solve(prob_logistica)

# ╔═╡ 76086a04-1015-4df6-8e5c-db0915dff568
md"""Lo primero que conviene observar es que la solución es una estructura particular. La podemos inspeccionar usando `propertynames`: """

# ╔═╡ c1ed79d4-a5c8-4753-835d-7efc3c63f553
propertynames(sol_logistica)

# ╔═╡ 7618e645-6f3b-4321-9cb0-498f23fded91
sol_logistica.u

# ╔═╡ fa0a20d4-3f1f-4814-be81-a3b13b5655bd
sol_logistica.t

# ╔═╡ 8462fcd7-e41c-47f0-8ed1-1bca1473b3a8
sol_logistica.alg

# ╔═╡ 7941ebb1-8215-4b49-972a-35e1e4e2afdf
sol_logistica.alg_choice

# ╔═╡ 61bab2bf-44d4-40e5-a9dd-3bc5314a16cd
sol_logistica.stats

# ╔═╡ bb868bed-195b-4650-bb3c-5230008dbc2c
md"""Es interesante observar que al obtener `sol_logistica` como output no nos mostró todos estos detalles, sino una tabla con la información más relevante: los valores de `t`y de `u`."""


# ╔═╡ 11be05e5-aed3-4e04-bae4-b20388f14b05
md"""`DifferentialEquations` está integrado com `Plots` de manera que para graficar la solución basta con hacer `plot(sol)`: """

# ╔═╡ e9dadc0f-3a26-4bc9-a99f-746df2d2c6e9
plot(sol_logistica)

# ╔═╡ 3ff323a0-28e5-4aef-a6fd-dbd38b40c10b
md"""Se puede observar que el dibujo es mucho más suave de lo que debería. En efecto: """

# ╔═╡ 81e56cd7-ab05-461a-ae20-712b29b0dace
plot(sol_logistica.t,sol_logistica.u)

# ╔═╡ 43d949b5-eb49-4969-8437-d67de483609c
md"""Lo que está pasando es que `sol_logistica` contiene información sobre cómo debe interpolarse la solución en los puntos no calculados y de ese modo `plot` puede evaluar la solución en valores a priori desconocidos. De hecho, también lo podemos hacer nosotros: """

# ╔═╡ 6836499a-b9e8-4b33-9a17-e43b9473a48d
sol_logistica(0.5)

# ╔═╡ 486ca031-b675-43a7-b4f5-df49f44b4a90
sol_logistica.t
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "a64821d38a524c132c0213e8d3643175aa040a58"
