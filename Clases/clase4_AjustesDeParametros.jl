...
# ╔═╡ bc232444-3088-11ee-3482-b38e99b0f5d0
using DifferentialEquations,DiffEqParamEstim,Plots, CSV, DataFrames,Optimization, OptimizationOptimJL

# ╔═╡ 34b6fae4-dbbe-4ff2-a060-515b1d348ddb
md"""# Ajuste de parámetros de EDOs

En general, los modelos no se corren *hacia adelante*. Un modelo es un patrón general al cual se espera que una determinada situación se ajuste, pero depende de parámetros que son desconocidos en casi cualquier caso particular. Por lo tanto, dados datos de una determinada situación y un modelo con el que se espera explicar esos datos, interesa ajustar los parámetros del modelo de manera tal que la dinámica que este presupone ajuste lo mejor posible los datos. 

Este ajuste corresponde a la resolución de un problema de *cuadrados mínimos*, típicamente no lineal. La librería `DiffEqParamEstim` nos da una interfaz para facilitar el planteo del problema, que podemos resolver luego con la librería `Optimization`. 
	"""

# ╔═╡ 83184be1-4081-421a-97f9-fd52eee1f458
md"""### Un ejemplo: datos de población. """

# ╔═╡ 9f9ad334-e1fa-401f-a814-fb2c5878f7f9
md"""Tomamos el archivo de datos de población mundial (descargado de la página del [Banco Mundial](https://data.worldbank.org/indicator/SP.POP.TOTL?downloadformat=csv)), y la cargamos como un DataFrame."""

# ╔═╡ 856bedf7-c2ba-4ba8-abc3-0b43444bec5a
df = CSV.read("pop_world.csv",DataFrame;header=4)

# ╔═╡ 814c57ca-b831-4ca0-a8ea-a06d84e3acd7
md"""Podemos, por ejemplo, extraer los datos para Argentina, pasarlos a un vector y graficarlos:"""

# ╔═╡ 922009e8-31e2-4574-bbc4-766ba94185d6
pais = "France"

# ╔═╡ 6029ecd5-7dbd-4577-b970-3d9b23fb42f2
sel = filter(row->row["Country Name"]==pais,df)

# ╔═╡ e4fc6663-c467-44ef-8297-bf5635092cb5
selv = collect(values(sel[1,5:end]))

# ╔═╡ f6c32777-2d2b-4b35-8b6f-3e47b57c0980
scatter(1960:2022,selv)

# ╔═╡ 4175802b-3f88-4b68-8d75-18d2b135ca90
md"""Estamos en condiciones de plantear un modelo. Por ejemplo, podemos comenzar con un simple modelo exponencial:"""

# ╔═╡ a32b1197-e997-4ea2-8e12-196a050cda83
mod_exp(x,μ,t) = μ[1]*x 

# ╔═╡ aacce66f-64f5-44cd-b102-582e0b253952
x0 = selv[1]

# ╔═╡ 49ff2285-5342-4483-911b-4af62964a64c
begin
	t = 1960:2022
	tspan = (t[1],t[end])
end

# ╔═╡ 4dab99f2-22a8-4380-93a9-e2e19cfff13d
prob_exp = ODEProblem(mod_exp,x0,tspan,[0.1])

# ╔═╡ 9ce9c78d-2e6a-4f94-8a3a-ddea9d4dc1ea
begin
...
# ╔═╡ c9744753-0cc8-440a-a0a9-e6536c06c049
func_exp = build_loss_objective(prob_exp,Tsit5(),L2Loss(t,selv),Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)

# ╔═╡ 258f3c9a-8bf9-4866-a8f4-e85f9e9e4754
optprob_exp = OptimizationProblem(func_exp,[0.1])#,lb=[0.],ub=[1.])

# ╔═╡ 8743735c-16fc-446f-b072-547d5495c164
plot(0:0.001:0.025,func_exp)

# ╔═╡ d48308b2-1754-4073-ae01-4bd209251cee
μ̄_exp = solve(optprob_exp,BFGS())

# ╔═╡ a6f4faea-6ede-473b-8455-c330fc564790
begin
	prob_exp₂ = remake(prob_exp,p=μ̄_exp)
	sol_exp₂  = solve(prob_exp₂)
	plot(sol_exp₂)
	scatter!(t,selv)
end
...
# ╔═╡ 13370ba2-0584-4d87-8e5c-00c4a1c4a4d2
mod_log(x,μ,t) = μ[1]*x*(1-x/μ[2])

# ╔═╡ 7b77e451-d891-4403-915e-ba01314a75cb
prob_log = ODEProblem(mod_log,x0,tspan,[0.1,100_000_000])

# ╔═╡ ac8c1449-9acb-4653-add3-e9e413d9ed0c
func_log = build_loss_objective(prob_log,Tsit5(),L2Loss(t,selv),  			                      Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)

# ╔═╡ 1233a3fd-713c-4130-8484-0de3d03ef477
optprob_log = OptimizationProblem(func_log,[0.1,50_000_000],lb=[0.,10_000_000],ub=[1.,500_000_000])

# ╔═╡ 6756d4f8-07ad-4622-8f70-451027feb465
μ̄_log = solve(optprob_log,BFGS())

# ╔═╡ e494a07e-1f24-4c9c-b154-6c4493eacb43
begin
	prob_log₂ = remake(prob_log,p=μ̄_log)
	sol_log₂  = solve(prob_log₂)
	plot(sol_log₂)
	scatter!(t,selv)
end

# ╔═╡ 87d30c2d-709a-430d-bfd0-d0ed9272e73a
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "a81e04ed6daba476d7d5a8e0fbabc03454fa0924"
...
git-tree-sha1 = "3cddbe0fad23df52587e675b3d71eea66b850cb9"
...
git-tree-sha1 = "79e0eff50c16554eb9d85b2e0464f25c53aaf561"
