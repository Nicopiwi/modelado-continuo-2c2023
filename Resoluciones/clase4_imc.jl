...
# ╔═╡ a1e7d886-4cb2-11ee-1fee-ad4577d34078
using DifferentialEquations,DiffEqParamEstim,Plots, CSV, DataFrames,Optimization, OptimizationOptimJL

# ╔═╡ 7a4d370c-33ae-46c0-b98e-65384086aaa7
df = CSV.read("pop_world.csv", DataFrame;header=4)

# ╔═╡ c73865f0-3f73-460d-8bc6-24ad746558d4
begin
	paises = ["Sweden", "Angola", "Argentina"]
end

# ╔═╡ b252bfdf-7c8b-4923-a572-b466b12e58bc
begin
	sel = Any[]
	for (index, pais) in enumerate(paises)
		push!(sel, filter(row->row["Country Name"]==pais,df))
	end
	 
end

# ╔═╡ 0b0f75a7-8a90-43c3-81a2-c391bc165653
begin
	selv = Any[]

	for (index, pais) in enumerate(paises)
		push!(selv, collect(values(sel[index][1,5:end])))
	end
end

# ╔═╡ cff53abd-911c-4d3f-89eb-11737675689d
begin

	scatter(1960:2022,selv[1], label="Sweden")
	scatter!(1960:2022,selv[2], label="Angola")
	scatter!(1960:2022,selv[3], label="Argentina")
	
end

# ╔═╡ 819e0b63-b82e-44b9-a2f9-c25518a15e80
mod_exp(x,μ,t) = μ[1]*x

# ╔═╡ 73d21317-3932-4950-90cc-a9aa8cc5df41
x0s = [selv[1][1], selv[2][1], selv[3][1]]

# ╔═╡ 43408cec-df61-4ced-9b06-000e94114f15
begin
	t = 1960:2022
	tspan = (t[1],t[end])
end

# ╔═╡ 6e15fad2-59ec-4592-b29f-53b37f5bfcd1
prob_exp = ODEProblem(mod_exp,x0s[1],tspan,[0.1])

# ╔═╡ 72b4e711-68cb-49e0-be4c-981e8a0e7329
costo_Sweden = build_loss_objective(prob_exp,Tsit5(),L2Loss(t,selv[1]),Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)

# ╔═╡ 92974b6c-d1c3-4ef0-8506-75892bb77baa
# ╠═╡ disabled = true
#=╠═╡
optprob_exp = OptimizationProblem(costo_Sweden,[0.01])
  ╠═╡ =#

# ╔═╡ f87b80cb-6e84-4434-a78e-f6ab3592d408
#=╠═╡
μ̄_exp = solve(optprob_exp,BFGS())
  ╠═╡ =#

# ╔═╡ 53e7b85f-24ff-43d6-8bbd-e9e033b66b11
#=╠═╡
μ̄_exp.original
  ╠═╡ =#

# ╔═╡ 55fe94ea-47e2-4fbb-a1a2-66db0d7d64a9
#=╠═╡
begin
...
# ╔═╡ 25c4edd3-cfe1-41df-b760-9479b235874e
md"""### Ejercicio 15"""

# ╔═╡ a639ba60-bfd2-401c-bd7c-08b16ffc1a39
...
# ╔═╡ a46a7112-64d8-4c5c-aba1-b9ecff2c0711
function mod_lv(x,p,t)
	b, s, k, d, e = p
	dx = zeros(2)
	dx[1] =(b-s*x[2])*x[1]
	dx[2] = (-d + e * s * x[1]) * x[2]
